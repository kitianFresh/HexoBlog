...
```
假设链表的平均长度是 n,有T(k) = 2\*T(k/2) + O(nk),这个递推公式可以直接算，T(k) = 2\*[2\*T(k/2^2)+nk/2] = 2^2T(k/2^2) + 2nk = ... = 2^xT(1) + xnk; 其中 2^x = k, x = lgk, T(1) = 1, 解得 T(k) = k + nklgk; 也可以使用Master主定理，时间复杂度 O(nklgk).
